...
Preflight runs `check`, `check:caddy`, the npm pack dry-run and tarball file-list check, offline packed-bin smoke, the real PTY interactive and non-interactive installer E2E, direct and outer-TLS-proxy browser E2E, the full managed lifecycle E2E (`node scripts/lifecycle-e2e.mjs` on a disposable root-owned machine or privileged systemd container: pre-install dormancy, trusted adoption, a failed upgrade that restores a pnpm `file:` spec offline, managed upgrade with session continuity, drift fail-closed plus the fixed three-step recovery, downgrade refusal, uninstall back to dormancy, the self-installed lifecycle, and dynamic proxied login-token origins without setup-state changes), tracked-file privacy checks, and gitleaks. Release pack vendors both official Caddy binaries with `scripts/pack-caddy-platform.mjs`, then creates the tarball with `--ignore-scripts` so lifecycle scripts do not rebuild or delete that tree. Setup never downloads Caddy. The lifecycle test refuses to run beside an existing managed installation unless the operator explicitly confirms a disposable host with `LIFECYCLE_DISPOSABLE_HOST=1`; it uses disposable files and does not touch a deployed Harness, port 3080, or host Caddy configuration.
