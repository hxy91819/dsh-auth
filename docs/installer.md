...
`upgrade` moves one healthy v2 system installation to the build of the running global CLI. The plan refuses missing installations, schema v1 records, output-mode records, interrupted setups, and any doctor error, including bundle drift (the doctor diagnostic carries the fixed recovery order: restore with `dsh plugin --profile <profile> add <recorded-spec>`, rerun doctor, then upgrade). The target must be exactly the CLI's package name, version, and build identity with a strictly higher version; same-version reinstalls and downgrades are refused before any mutation. Execution journals the transaction inside the ownership record (`upgrade` block with phase `bundle`, `caddy`, `quiescing`, or `services`), updates and validates the target bundle, Caddy, and environment, then stops the recorded DSH service before creating a root-owned `0600` authentication-state snapshot. Only after the state is quiescent does it enter `services`, start the target DSH, restart the owned Caddy unit, and finalize the record. Administrator credentials and the session secret are never changed. A successful upgrade preserves the live authentication state and removes the snapshot. Any failure rolls the bundle, binary, environment marker, record, and services back to the recorded build; if the target service may have written a newer state schema, rollback first stops it and restores the snapshot so the recorded bundle can read the state. A rollback that cannot restore the recorded build identity fails closed with `UPGRADE_RESTORE_BUILD_MISMATCH` and the same fixed recovery order. A crash leaves the journal and any required snapshot behind; the next `upgrade` first reconciles by rolling back, then proceeds. Interactive use asks for the exact word `upgrade`; non-interactive use requires `--non-interactive` together with `--authorize-upgrade`, and `--dry-run` prints the secret-free plan.
...
- Systemd service names, profile names, absolute paths, upstreams, listen addresses, DNS names, public IP addresses, and certificate paths are validated before discovery or rendering.
