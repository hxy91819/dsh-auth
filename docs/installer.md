...
Installer changes require observable tests for the affected success, refusal, and rollback paths. At minimum run the repository `check`, isolated `check:nginx`, `npm pack --dry-run`, and `scripts/pack-smoke.mjs` against the produced tarball. Before push, scan tracked files and Git metadata, run gitleaks, and inspect the tarball file list. Do not test against the active host Nginx include, reload its service, or bind its deployed port; disposable Nginx prefixes and ports are sufficient.
