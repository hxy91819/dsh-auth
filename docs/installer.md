...
- HTTPS requires an explicit server name. Automatic TLS rejects certificate parameters; manual TLS requires both certificate and key, exposed to Caddy only through systemd credentials. Trusted-network HTTP requires an explicit private/loopback literal address and disables secure cookies and HSTS. Plain HTTP on another intranet literal address additionally requires `--authorize-insecure-address`; the acknowledgment enters the persistent request and fingerprint only when set, so records written before the flag existed stay `unchanged`. Container-mode token issue still requires a private/loopback HTTP `--public-origin`. `--behind-tls-proxy` requires HTTP on loopback, requires HTTPS forwarding metadata, and keeps secure cookies and HSTS because the browser-facing transport is HTTPS.
