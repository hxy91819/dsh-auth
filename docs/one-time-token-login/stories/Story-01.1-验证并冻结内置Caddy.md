...
- [ ] 固定 Caddy 版本、来源、许可证、校验和与可复现临时运行入口。
- [ ] 用最终 verify 协议通过 HTTP、实时连接、下载和安全矩阵。
- [ ] 验证 automatic 与 manual TLS、HTTP/2 和续期 Cookie 转发。
- [ ] 与同机 Nginx 比较吞吐、p95、5xx、SSE 和 WebSocket 稳定性。
- [ ] 记录标准配置能力缺口并冻结正式安装所需配置契约。
