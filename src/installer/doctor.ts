...
export function buildDoctorPlan(host: InstallerHost, stateFile = DEFAULT_STATE_FILE): { readonly plan: InstallationPlan; readonly state?: InstallState } {
  ...
  if (state === undefined) {
    ...
    let serviceIdentity: DshServiceDiscovery | undefined
    ...
    if (host.regularFile(state.paths.environmentFile)) {
      const environment = host.readFile(state.paths.environmentFile)
      if (/^DSH_AUTH_(?:PASSWORD_HASH|SESSION_SECRET)=/mu.test(environment)) {
    ...
    try {
      serviceIdentity = discoverDshService(host, state.dshService, { dshHome: state.dshHome, dshExecutable: state.dshExecutable })
      if (serviceIdentity.activeState !== 'active') diagnostics.push({ code: 'DSH_SERVICE_INACTIVE', severity: 'error', message: `DSH service ${state.dshService} is not active.` })
    } catch (error) {
      if (error instanceof InstallerError) {
        if (error.diagnostics.length > 0) diagnostics.push(...error.diagnostics)
        else diagnostics.push({ code: 'DSH_SERVICE_INVALID', severity: 'error', message: error.message })
      }
      else throw error
    }
    if (serviceIdentity !== undefined) {
      const expectedOwners = new Map<string, readonly [number, number]>([
        [state.paths.configDirectory, [0, serviceIdentity.gid]],
        [state.paths.stateFile, [0, 0]],
        [state.paths.environmentFile, [0, serviceIdentity.gid]],
        [state.paths.passwordHashFile, [0, serviceIdentity.gid]],
        [state.paths.sessionSecretFile, [0, serviceIdentity.gid]],
        [state.paths.sessionDirectory, [serviceIdentity.uid, serviceIdentity.gid]],
    ...
    if (state.profilePackageInstalledByDshAuth) {
      ...
      if (!packagePresent) diagnostics.push({ code: 'PROFILE_PACKAGE_MISSING', severity: 'error', message: `The recorded DSH profile ${state.request.profile} no longer contains the dsh-auth bundle.` })
    }
  }
  ...
  if (systemctl === undefined) diagnostics.push({ code: 'SYSTEMD_MISSING', severity: 'error', message: 'systemd is unavailable.' })
    ...
    const active = host.run({ executable: systemctl, args: ['is-active', '--quiet', 'nginx.service'] })
    if (active.status !== 0) diagnostics.push({ code: 'NGINX_SERVICE_INACTIVE', severity: 'error', message: 'nginx.service is not active.' })
  }
