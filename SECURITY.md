...
Trusted team preview accounts are separate login identities that share one Harness instance authority. They are appropriate only for mutually trusted collaborators. dsh-auth exposes account/session activity for those trusted users, including active browser-session counts and last-seen timestamps, can show current Harness-session participants recorded by dsh-auth collaboration metadata, and can show/send visible prompt author labels for browser-submitted prompts. These labels, participant lists, and prompt counts are user-visible collaboration affordances, not immutable Harness metadata or an audit log. They do not provide private conversations, per-workspace access control, native or audit-grade prompt/controller attribution inside Harness, per-user tool permissions, or audit-grade attribution for model/tool actions.
...
- A login-page GET reuses a still-valid CSRF cookie so separate tabs do not invalidate one another. If a login POST has a stale or invalid CSRF pair, the server keeps the 403 denial, returns a fresh no-store login form, and requires the operator to submit it again; it never replays the submitted password.
...
Runtime security events intentionally exclude submitted usernames, password hashes, raw login tokens, cookies, CSRF values, session identifiers, request bodies, and complete request URLs. CSRF denial diagnostics may distinguish a missing form value, missing cookie, invalid or expired signed cookie, and form/cookie mismatch using fixed reason labels only; they never record the token or cookie. Application events use an irreversible deployment-scoped client identifier and limit warnings and errors to 60 events per minute, with a suppression summary in the next active window. Caddy skips routine SPA and API access logs, logs only security-sensitive authentication paths, and omits request and response headers. Its restricted access-log file rolls at 10 MiB and keeps at most three compressed archives for seven days; the remaining network addresses and paths still require controlled access and redaction before external sharing.
