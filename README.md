...
Ordinary browser-submitted prompts receive a visible author line such as `👤 teammate · member`. This makes multiple people's turns distinguishable inside one Harness conversation; until Harness provides native message-author metadata or a stable message-row avatar slot, the label is part of the model-visible prompt text rather than an audit-grade immutable author field. Slash commands are not rewritten.
