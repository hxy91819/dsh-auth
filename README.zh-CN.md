...
预览开启后，`/auth/session`、`/auth/account` 和 `/auth/admin/accounts` 会展示团队活动视图：每个账号都有首字母头像、角色、状态、活跃浏览器会话数和最近活动时间。侧栏身份胶囊也会显示当前有多少可信团队账号拥有活跃浏览器会话。这是 dsh-auth 的会话可见性，不是 Harness 对话 ACL 或工作区归属模型。
