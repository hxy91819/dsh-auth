...
预览开启后，`/auth/session`、`/auth/account` 和 `/auth/admin/accounts` 会展示团队活动视图：每个账号都有首字母头像、角色、状态、活跃浏览器会话数和最近活动时间。侧栏身份胶囊和输入区身份条会周期刷新，并显示当前有多少可信团队账号拥有活跃浏览器会话。这是 dsh-auth 的会话可见性，不是 Harness 对话 ACL 或工作区归属模型。
...
浏览器提交的普通 prompt 会在文本前添加可见的发言人行，例如 `👤 teammate · member`。Harness 输入框上方也会显示当前账号、角色和可信团队在线数，让发送前能确认本次发言会带哪个身份。这让同一 Harness 会话中的多人发言可以被区分；但在 Harness 提供原生消息作者元数据或稳定的消息行头像插槽前，这个标识会进入模型上下文，不是审计级别的不可篡改作者字段。斜杠命令不会被改写。
