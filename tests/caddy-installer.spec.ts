...
describe('Caddy installer contract', () => {
  it('renders HTTPS config with Admin API off and public /auth/verify denied', () => {
    ...
    expect(rendered).toContain('output file "/var/lib/dsh-auth-caddy/access.log"')
    expect(rendered).toContain('roll_size 10MiB')
    expect(rendered).toContain('roll_keep 3')
    expect(rendered).toContain('roll_keep_for 168h')
    expect(rendered).toContain('format filter')
    expect(rendered).toContain('request>headers delete')
